    ...
    <div className="">
      ...
      <div className="md:hidden">
        <div className="flex flex-wrap gap-y-4">
          {categories.map((category) => (
            <div key={category.id} className="basis-1/2 pl-2">
              <div className="flex-1 relative ml-2.5" key={category.id}>
                <div className="absolute bottom-4 -left-3 bg-white font-medium text-lg xl:text-xl py-1 xl:py-1.5 px-2 xl:px-5 [box-shadow:0px_1px_7px_0px_#00000091] flex w-full justify-between flex-wrap gap-x-2">
                  <p className="text-nowrap text-sm">{category.name}</p>
                  <Link
                    className="text-sm md:text-md xl:text-lg xl:ml-4 text-[#14B1F0]"
                    href="/shop"
                  >
                    Shop
                  </Link>
